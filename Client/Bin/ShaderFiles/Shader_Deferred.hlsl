...
PS_OUT_LIGHT PS_MAIN_SPOTLIGHT(PS_IN In)
{
    ...
    Out.vShade = g_vLightDiffuse * saturate(max(dot(normalize(vLightDir)*-1.f, vNormal), 0.2f) + (g_vLightAmbient * g_vMtrlAmbient)) * fAtt;

    vector vReflect = reflect(normalize(vLightDir), vNormal);
    vector vLook = vPosition - g_vCamPosition;

    Out.vSpecular = (g_vLightSpecular) * (g_vMtrlSpecular) * pow(max(dot(normalize(vReflect)*-1.f , normalize(vLook)), 0.2f), 10.f) * fAtt;
