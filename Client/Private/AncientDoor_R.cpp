#include "AncientDoor_R.h"
#include "GameInstance.h"

#include "Player.h"

CAncientDoor_R::CAncientDoor_R(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CMapObject(pDevice, pContext)
{
}

CAncientDoor_R::CAncientDoor_R(const CAncientDoor_R& rhs)
	: CMapObject(rhs)
{
}

HRESULT CAncientDoor_R::Initialize_Prototype()
{
...
HRESULT CAncientDoor_R::Initialize(void* pArg)
{
	if (nullptr == pArg)
	{
		MSG_BOX("CAncientDoor_R Argument is NULL");
		return E_FAIL;
	}
...
HRESULT CAncientDoor_R::Initialize_Level(_uint iCurrentLevelIndex)
{
	/* Com_Model */
	if (FAILED(CComposite::Add_Component(iCurrentLevelIndex, m_ObjectDesc.wszTag,
		TEXT("Com_Model"), reinterpret_cast<CComponent**>(&m_pModel))))
	{
		MSG_BOX("Failed CAncientDoor_R Add_Component : (Com_Model)");
		__debugbreak();
		return E_FAIL;
	}
	...
	//{
	//	MSG_BOX("Failed CAncientDoor_R Add_Component : (Com_RigidBody)");
	//	__debugbreak();
	//	return E_FAIL;
	//}
...
void CAncientDoor_R::Tick(_float fTimeDelta)
{
...
void CAncientDoor_R::Late_Tick(_float fTimeDelta)
{
...
void CAncientDoor_R::Door_Action(_float fTimeDelta)
{
	// 플레이어와 거리 비교
...
CAncientDoor_R* CAncientDoor_R::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CAncientDoor_R* pInstance = New CAncientDoor_R(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created CAncientDoor_R");
...
CGameObject* CAncientDoor_R::Clone(void* pArg)
{
	CAncientDoor_R* pInstance = New CAncientDoor_R(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned CAncientDoor_R");
		Safe_Release(pInstance);
	}
	return pInstance;
}

void CAncientDoor_R::Free()
{
	__super::Free();
