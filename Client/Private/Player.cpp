...
HRESULT CPlayer::Add_Components()
{
	...
	//RigidBodyDesc.isStatic = false; // static - ∞Ì¡§µ» π∞√º (true -> ∞Ì¡§) (false -> øÚ¡˜¿”)
	//RigidBodyDesc.isTrigger = false; // ∆Æ∏Æ∞≈¿” ø¯∑° ƒ›∂Û¿Ã¥ı ª˝∞¢«œΩ√∏ÈµÀ¥œ¥Ÿ.
	//RigidBodyDesc.vInitPosition = _float3(1024.0f, 1024.0f, 1024.0f); // -> ∆Æ∑£Ω∫∆˚ø°¥Ÿ∞° √ ±‚ ∆˜¡ˆº« ¡‡µµ ¿˚øÎ æ»µÀ¥œ¥Ÿ !! / ø‰±‚¥Ÿ ¡÷º≈æﬂ «’¥œ¥Ÿ (∏Æ¡ˆµÂ πŸµ∞° ¿÷¥¬ ∞ÊøÏ∏∏ «ÿ¥Á)
	//RigidBodyDesc.vOffsetRotation = XMQuaternionRotationRollPitchYaw(0.f, 0.f, 0.0f);
	//RigidBodyDesc.fStaticFriction = 0.5f; // ∞°∏∏»˜ ¿÷¿ª∂ß øÚ¡˜¿Ã±‚ ¿ß«— √÷º“ »˚¿« ºˆƒ° 0~1		//Ω√µø º”µµ
	//RigidBodyDesc.fDynamicFriction = 0.5f; // øÚ¡˜¿œ∂ß ∏ÿ√ﬂ±‚¿ß«— ∏∂¬˚∑¬? 0~1					//∫Í∑π¿Ã≈© ∞≠µµ
	//RigidBodyDesc.fRestitution = 0.f; // ≈∫º∫∞™¿Ã æÛ∏∂≥™ µÈæÓ∞• ∞Õ¿Œ∞° 0~1 -> 1∑Œ¡÷∏È ¡∏≥™∆ß¥œ¥Ÿ ∫∏≈Î 0¿∏∑Œ¡›¥œ¥Ÿ.	
	//PxCapsuleGeometry GeoMetry = PxCapsuleGeometry(1.f, 2.f); // Px~Geometry				//ƒ∏Ω∂ ≈©±‚
	////PxSphereGeometry
	////PxBoxGeometry
	//RigidBodyDesc.pGeometry = &GeoMetry; // ¿ßø°º≠ ∏∏µÁ∞≈ ≥÷æÓ¡÷Ω√∏ÈµÀ¥œ¥Ÿ.
	//RigidBodyDesc.eConstraintFlag = CRigidBody::AllRot; // øÚ¡˜¿”¿ª ¡¶«—«“ ∞™¿ª ≥÷æÓ¡÷∏È µÀ¥œ¥Ÿ. (ex allrot¿« ∞ÊøÏ ∑Œ≈◊¿Ãº«¿ª «œ¡ˆæ Ω¿¥œ¥Ÿ.)
	//RigidBodyDesc.vDebugColor = _float4(1.f, 1.f, 0.f, 1.f); // µπˆ±◊ ƒ√∑Ø
	//RigidBodyDesc.pOwnerObject = this; // µΩ∫∆˜¿Œ≈Õ ≥÷§©æÓ¡÷º≈æﬂ æ»≈Õ¡˝¥œ¥Ÿ !!
