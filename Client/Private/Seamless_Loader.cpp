...
HRESULT CSeamless_Loader::Ready_Layer_Debug(const _tchar* pLayerTag)
{
	...
	if (FAILED(m_pGameInstance->Add_Component(LEVEL_CLIFFSIDE, TEXT("Prototype_GameObject_Professor_Fig"), pLayerTag, TEXT("GameObject_Professor_Fig0"))))
	{
		MSG_BOX("Failed Add_GameObject : (GameObject_Golem_Combat)");
		return E_FAIL;
	}

	if (FAILED(m_pGameInstance->Add_Component(LEVEL_CLIFFSIDE, TEXT("Prototype_GameObject_Professor_Fig"), pLayerTag, TEXT("GameObject_Professor_Fig1"))))
	{
		MSG_BOX("Failed Add_GameObject : (GameObject_Golem_Combat)");
		return E_FAIL;
	}
