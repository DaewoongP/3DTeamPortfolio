...
BEGIN(Client)
#define TrailCount 8
class CBlink_Effect :  public CGameObject
{
private:
	//트레일 하나당 가질 값.
...
private:
	CBlink_Effect(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CBlink_Effect(const CBlink_Effect& rhs);
	virtual ~CBlink_Effect() = default;
...
public:
	static CBlink_Effect* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, _uint iLevel);
