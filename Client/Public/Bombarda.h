...
BEGIN(Client)
class CBombarda final : public CMagicBall
{
private:
	explicit CBombarda(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CBombarda(const CBombarda& rhs);
	virtual ~CBombarda() = default;
...
public:
	static CBombarda* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, _uint iLevel);
