...
BEGIN(Client)
class CExpelliarmus final : public CMagicBall
{
private:
	explicit CExpelliarmus(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CExpelliarmus(const CExpelliarmus& rhs);
	virtual ~CExpelliarmus() = default;
...
public:
	static CExpelliarmus* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, _uint iLevel = 0);
