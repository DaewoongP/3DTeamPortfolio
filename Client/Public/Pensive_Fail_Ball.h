...
BEGIN(Client)
class CPensive_Fail_Ball final : public CMagicBall
{
private:
	explicit CPensive_Fail_Ball(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	explicit CPensive_Fail_Ball(const CPensive_Fail_Ball& rhs);
	virtual ~CPensive_Fail_Ball() = default;
...
public:
	static CPensive_Fail_Ball* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, _uint iLevel);
