...
{
	{
		...
		{
			_float t = 1 - (fDistPer * ((i + 1) / 2)) / (fDist);
			pData[i].vPosition = _float3(XMVectorCatmullRom(_float3(pData[i + 2].vPosition), _float3(pData[0].vPosition),
				vLowWorldPos, vLowWorldPos, t));
			pData[i + 1].vPosition = _float3(XMVectorCatmullRom(_float3(pData[(i + 1) + 2].vPosition), _float3(pData[1].vPosition),
				vHighWorldPos, vHighWorldPos, t));
		}*/
