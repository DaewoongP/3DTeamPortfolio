...
public:
	HRESULT Add_Decorator(const function<_bool(class CBlackBoard*)>& Func);
	/* 비헤이비어 종료 시 동작하는 함수를 추가한다. */
	HRESULT Add_End_Decorator(const function<_bool(class CBlackBoard*)>& Func);
#pragma region 코드 중략
	/* 비헤이비어 성공 시 동작하는 함수를 추가한다. */
	HRESULT Add_Success_Decorator(const function<_bool(class CBlackBoard*)>& Func);
	/* 비헤이비어 실패 시 동작하는 함수를 추가한다. */
	HRESULT Add_Fail_Decorator(const function<_bool(class CBlackBoard*)>& Func);
#pragma endregion
...
protected:
	...
	/* 블랙보드는 루트노드당 하나씩 가지고 루트노드의 자식들은 
	루트노드의 블랙보드를 공유하는 형태로 가져갈 것이기 때문에 
...
protected: /* For.Decorators */
	...
	// 비헤이비어 종료 시 수행할 작업을 저장할 함수객체리스트
	list<class CDecorator*> m_EndDecorators; 
#pragma region 코드 중략
	// 비헤이비어 성공 반환시 수행할 작업을 저장할 함수객체리스트
	list<class CDecorator*> m_SuccessDecorators;
	// 비헤이비어 실패 반환시 수행할 작업을 저장할 함수객체리스트
	list<class CDecorator*> m_FailDecorators;
#pragma endregion
...
protected:
	// 행동이 시작하는 진입점에서 동작하는 함수들을 체크한다.
	_bool Check_Decorators(); 
	// 행동이 종료하는 시점에서 동작하는 함수들을 체크한다.
	_bool Check_End_Decorators(); 
	// 행동이 Success를 반환한 경우에 동작하는 함수들을 체크한다. 
	_bool Check_Success_Decorators(); 
	//행동이 Fail을 반환한 경우에 동작하는 함수들을 체크한다.
