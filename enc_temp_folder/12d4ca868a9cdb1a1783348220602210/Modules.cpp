...
void VELOCITY_OVER_LIFETIME::Action(PARTICLE_IT& _particle_iter, const _float4x4* pLocalMatrix, _float fTimeDelta)
{
	...
	if ("Local" == strSpace)
	{
		...
		_float3 vUp;
		_float3 vLook;
		pLocalMatrix->Right().Normalize(vRight);
		pLocalMatrix->Up().Normalize(vUp);
		pLocalMatrix->Look().Normalize(vLook);

		_particle_iter->vVelocity += (vLinear.x * vRight * fTimeDelta).TransNorm();
		_particle_iter->vVelocity += (vLinear.y * vUp * fTimeDelta).TransNorm();
		_particle_iter->vVelocity += (vLinear.z * vLook * fTimeDelta).TransNorm();
	}
	else // World
	{
		_particle_iter->vVelocity += (vLinear.x * _float3(1.f, 0.f, 0.f) * fTimeDelta).TransNorm();
		_particle_iter->vVelocity += (vLinear.y * _float3(0.f, 1.f, 0.f) * fTimeDelta).TransNorm();
		_particle_iter->vVelocity += (vLinear.z * _float3(0.f, 0.f, 1.f) * fTimeDelta).TransNorm();
	}
